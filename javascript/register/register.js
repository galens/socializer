		...
		onsubmitcomplete : function(data,textStatus){
			...
			if(textStatus == "success"){
					if(data.result == "1"){
						...
						htmlstr += "<h1>Confirm your registration</h1><p>You are just one step away from finishing registration! A link has been sent to your email account for you to confirm.</p>";
						...
						$('#ajaxld').hide();
						$('#_register_btt').show();
						$('#reg').html("<span>an error occurred</span>");
					}
			...
			else if(textStatus == "error")//TODO - it can be of more types!
			{
					alert("error ajax");
					$('#ajaxld').hide();
					$('#reg').innerHTML = "<span>an error occurred</span>";
			}
		}
