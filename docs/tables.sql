...
DROP TABLE IF EXISTS scheduled_twitter_events;
#
...
CREATE TABLE users(
		...
		premium 					tinyint(1) not null default 0,			# 0 for regular, 1 for premium
...
CREATE TABLE twitter(
		...
		dm_new_follow				tinyint(1) not null default 0,			# 0 for off, 1 for on
		re_follow					tinyint(1) not null default 0,			# 0 for off, 1 for on
		purge_dm					tinyint(1) not null default 0,			# 0 for off, 1 for on
		foreign key(pk_user) 		references users(pk_user),
		unique index(oauth_token),
		primary key(twitter_id)
)type=innodb DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

#
#  Table structure for scheduled_tweets_events table
#
CREATE TABLE scheduled_twitter_events(
		scheduled_twit_id			int unsigned not null auto_increment,
		pk_user 					int unsigned not null,
		twitter_id		 			int unsigned not null,
		dm_screenname 				varchar(200) default null,
		event_body					text not null,
		scheduled_twit_date			bigint unsigned not null,
		content_type				int(2) not null default 0,
		foreign key(pk_user) 		references twitter(pk_user),
		primary key(scheduled_twit_id)
)type=innodb DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

#
#  Table structure for twitter_followers table
#
CREATE TABLE twitter_followers(
		twitter_followers_id		int unsigned not null auto_increment,
		twitter_id		 			int unsigned not null,
		pk_user 					int unsigned not null,
		followers_data				text not null,
		lastupdate					bigint unsigned not null,					
		foreign key(twitter_id) 	references twitter(twitter_id),
		primary key(twitter_followers_id)
)type=innodb DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

#
#  Table structure for twitter_friends table
#
CREATE TABLE twitter_friends(
		twitter_friends_id			int unsigned not null auto_increment,
		twitter_id		 			int unsigned not null,
		pk_user 					int unsigned not null,
		friends_data				text not null,
		lastupdate					bigint unsigned not null,					
		foreign key(twitter_id) 	references twitter(twitter_id),
		primary key(twitter_friends_id)
)type=innodb DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

#
#  Table structure for direct_message table
#
